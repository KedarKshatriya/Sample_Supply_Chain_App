...
contract supply_Chain {
   ...
   /* function getProduct_track(uint prod_id)  public  returns (track_product[]) {
    ...
    function getProduct_trackindex(uint trck_id)  public  returns (uint,uint,address,uint,string,string) {
        
        track_product t = tracks[trck_id];
        product p = products[t._product_id];
         return (t._product_id,t._owner_id,t._product_owner,t._timeStamp,p._product_name,p._product_specs);
    }
